...
def main():
    ...
    for split in range(n_splits):
        ...
        for i in tqdm(params):
            ...
            for split_id in range(multiplier):
                if any([j in i for j in vertical]):
                    ...
                    new_params[split_id][i] = params[i][split_id * new_size : (split_id+1) * new_size].clone()
                    assert new_params[split_id][i].numel() == params[i].numel() // multiplier
                    print("reduced")
        ...
        for split_id in range(multiplier):
            ...
            torch.save(new_params[split_id], f"/home/users/giovannipuccetti/Models/65B_spread_64/consolidated.{n:02}.pth")
